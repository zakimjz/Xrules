...
int Dbase_Ctrl_Blk::Class=0;
vector<int> Dbase_Ctrl_Blk::classidx(100,0);
...
void Dbase_Ctrl_Blk::get_first_blk()
{
   ...
   if (binary_input){
      ...
      //cout << "CUR BLK " << cur_blk_size << endl;
      if (cur_blk_size < 0){
         cerr << "problem in get_first_blk" << cur_blk_size << endl;
         exit(1);
      }
      else if (cur_blk_size < buf_size){
         //reset the file pointer to end
         fd.clear();
         fd.seekg(0,ios::end);
         //cout << "ED " << fd.tellg() << endl;
      }
   }
   cur_buf_pos = 0;
}
...
int Dbase_Ctrl_Blk::get_next_trans ()
{
   ...
   if (binary_input){
      if (cur_buf_pos+TRANSOFF >= cur_blk_size ||
         ...
         int curpos = fd.tellg();
         if (curpos == endpos) readall = 1;      
         //cout<< "END POS: " << curpos << " " << endpos << endl;
         if (!readall){
      ...
      if (!readall){
         Class = buf[cur_buf_pos+TRANSOFF-4];
         Cid = buf[cur_buf_pos+TRANSOFF-3];
...
void Dbase_Ctrl_Blk::get_next_trans_ext()
{
   ...
   else if (res < (buf_size-cur_blk_size)){
      //reset the file pointer to end
      fd.clear();
      fd.seekg(0,ios::end);
      //cout << "ED " << fd.tellg() << endl;
...
void Dbase_Ctrl_Blk::make_vertical(){
  ...
  for (i=0, pi=0; i < TransSz; i++){
    ...
    if (Cid >= classidx.size()) classidx.resize(Cid+1,0);
    classidx[Cid] = Class;
    idnode nn(Cid, pi,pj);
